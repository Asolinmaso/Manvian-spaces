  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full ${
        inBanner ? "bg-transparent" : "bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60"
      }`}
    >
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-[100px] py-4">
        <nav className="w-full max-w-[1240px] mx-auto flex items-center justify-between h-16 lg:h-[103px] px-4 lg:px-6 bg-white border border-primary rounded-[32px] lg:rounded-[64px]">
          ...
          <Image
            ...
            width={188}
            height={61}
            className="h-auto w-[140px] lg:w-[188px]"
          />
        </Link>
          ...
          <div className="hidden lg:block">
            <Link
              href="/contact"
              className="flex items-center justify-center px-6 py-4 bg-primary text-white font-semibold text-[20px] rounded-[48px] shadow-[0px_4px_10px_rgba(0,0,0,0.25)] hover:opacity-90 transition-opacity"
            >
              Contact Us
            </Link>
          </div>
              ...
              <div className="flex flex-col gap-3">
                {links.map((link) => (
                  <Link
                    ...
                    className={`px-4 py-3 rounded-2xl text-[18px] leading-6 ${
                      link.isActive
